...
const LP0_HOLDER = '0x9bb3a2876c09b7a51dd9fcaded501ffc393b08ea';
const LP0_GAUGE = '0x96a24aB830D4ec8b1F6f04Ceac104F1A3b211a01';
...
const LP1_GAUGE = '0xC01E2ff20501839db7B28F5Cb3eD2876fEa3d6b1';
...
describe("base: Aerodrome gauge Testing", function () {
    before("Initial set up", async function () {
        ...
        pluginFactory = await ethers.getContractAt("AeroGaugePluginFactory", pluginFactoryContract.address);
        console.log("- Plugin Factory Initialized");

        // initialize LP0
        await pluginFactory.createPlugin(LP0.address, 'vLP-AERO/WETH');
        plugin0 = await ethers.getContractAt("contracts/plugins/base/AeroGaugePluginFactory.sol:AeroGaugePlugin", await pluginFactory.last_plugin());

        // initialize LP1
        await pluginFactory.createPlugin(LP1.address, 'sLP-MAI/USDbC');
        plugin1 = await ethers.getContractAt("contracts/plugins/base/AeroGaugePluginFactory.sol:AeroGaugePlugin", await pluginFactory.last_plugin());
    ...
    it("LPGauge data ", async function () {
        ...
        console.log("Claimable AERO: ", await LP0Gauge.connect(owner).earned(plugin0.address));
        console.log();

        console.log("LP1Gauge");
        console.log("Claimable AERO: ", await LP1Gauge.connect(owner).earned(plugin1.address));
        console.log();
    });
