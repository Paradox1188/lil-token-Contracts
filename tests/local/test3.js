...
describe("local: test3", function () {
    ...
    it("User1 burns 100 OTOKEN for voting power", async function () {
        console.log("******************************************************");
        await OTOKEN.connect(user1).approve(VTOKEN.address, await OTOKEN.balanceOf(user1.address));
        await VTOKEN.connect(user1).burnFor(user1.address, await OTOKEN.balanceOf(user1.address));
    });
    ...
    it("Deploy new Voter contract and swap out for old one coverage", async function () {
        ...
        const gaugeFactoryArtifact = await ethers.getContractFactory("GaugeFactory");
        const gaugeFactoryContract = await gaugeFactoryArtifact.deploy(owner.address);
        gaugeFactory1 = await ethers.getContractAt("GaugeFactory", gaugeFactoryContract.address);
        console.log("- GaugeFactory Initialized");

        //initialize BribeFactory
        const bribeFactoryArtifact = await ethers.getContractFactory("BribeFactory");
        const bribeFactoryContract = await bribeFactoryArtifact.deploy(owner.address);
        bribeFactory1 = await ethers.getContractAt("BribeFactory", bribeFactoryContract.address);
        console.log("- BribeFactory Initialized");

        // initialize Voter
        const voterArtifact = await ethers.getContractFactory("Voter");
        const voterContract = await voterArtifact.deploy(VTOKEN.address, gaugeFactory1.address, bribeFactory1.address);
        voter1 = await ethers.getContractAt("Voter", voterContract.address);
        console.log("- Voter Initialized");

        // initialize Multicall
        const multicallArtifact = await ethers.getContractFactory("Multicall");
        const multicallContract = await multicallArtifact.deploy(voter1.address, BASE.address, TOKEN.address, OTOKEN.address, VTOKEN.address, rewarder.address);
        multicall1 = await ethers.getContractAt("Multicall", multicallContract.address);
        console.log("- Multicall Initialized");
    ...
    it("Deploy LP0 and LP1 plugins and add to new voter", async function () {
        ...
        const PluginFactoryArtifact2 = await ethers.getContractFactory("MockPluginFactory");
        const PluginFactoryContract2 = await PluginFactoryArtifact2.deploy(voter1.address);
        pluginFactory2 = await ethers.getContractAt("MockPluginFactory", PluginFactoryContract2.address);
        console.log("- PluginFactory Initialized");

        await pluginFactory2.createLPMockPlugin('LP2', 'TEST4', 'BASE1');
        plugin4 = await ethers.getContractAt("contracts/plugins/local/MockPluginFactory.sol:MockPlugin", await pluginFactory2.last_plugin());
        console.log("- Plugin2 Initialized")

        await pluginFactory2.createLPMockPlugin('LP3', 'TEST5', 'BASE1');
        plugin5 = await ethers.getContractAt("contracts/plugins/local/MockPluginFactory.sol:MockPlugin", await pluginFactory2.last_plugin());
        console.log("- Plugin3 Initialized")

        LP2 = await ethers.getContractAt("contracts/plugins/local/MockPluginFactory.sol:ERC20Mock", await plugin4.getUnderlyingAddress());
        LP3 = await ethers.getContractAt("contracts/plugins/local/MockPluginFactory.sol:ERC20Mock", await plugin5.getUnderlyingAddress());
        ...
        gauge4 = await ethers.getContractAt("contracts/GaugeFactory.sol:Gauge", Gauge4Address);
        bribe4 = await ethers.getContractAt("contracts/BribeFactory.sol:Bribe", Bribe4Address);
        ...
        gauge5 = await ethers.getContractAt("contracts/GaugeFactory.sol:Gauge", Gauge5Address);
        bribe5 = await ethers.getContractAt("contracts/BribeFactory.sol:Bribe", Bribe5Address);
    ...
    it("user0 votes on plugins a", async function () {
        console.log("******************************************************");
        await voter1.connect(user0).vote([plugin4.address, plugin5.address],[ten, ten]);
    });
    ...
    it("Minter Coverage Testing", async function () {
        console.log("******************************************************");
        await expect(minter.setWeeklyRate(threeThousand)).to.be.revertedWith("Minter__WeeklyRateTooHigh");
        await expect(minter.setTailRate(0)).to.be.revertedWith("Minter__TailRateTooLow")
